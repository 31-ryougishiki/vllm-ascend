...
| A3 现场实测（2026-09-12，preflight） | `vllm=/usr/local/python3.12.13/bin/vllm`（**与 share 站点的 py3.11.10 不是同一套 vllm 构建**）、`model=/opt/its/model/GLM-5.2-W4A8C8`、`repo=/opt/its/z30055003/vllm-ascend`、`vendor=none`、`tp=16`；`--preflight` 两配置 OK。⚠️ **两处待确认**：① 这台机器上默认权重是 **W4A8C8**，而我们至今的全部精度证据都来自 **w4a4c8-mxfp4**（layer 0 MLP = W8A8_MXFP8）；换量化方案后"GEMM 已排除"不能直接外推，且复现器 `--random-weight` 造的是 fp8+e8m0 权重，对 W4A8 不成立（只能靠真实 `w` dump）。② `vendor=none`：若这套 vllm-ascend 依赖 `_cann_ops_custom` 的 vendor set_env，第一轮会在加载早期失败 → 那时 `export VENDOR_SET_ENV=<path>` |
