...
def update_cos_sin(positions):
    ...
    num_tokens = positions.size(0)
    # DIAG bisect inside update_cos_sin (removed/gated since 2026-08-07):
    # sync after each sub-step to find where the lazy device work originates.
    # Each block is additive.  Re-enable with VLLM_DIAG_COS_SYNC=1.
    import os as _os_diag
    if _os_diag.environ.get("VLLM_DIAG_COS_SYNC", "") == "1":
