...
def test_ascend_causal_conv1d_optimized(dim, width, extra_state_len, seq_len, has_bias,
...
def test_causal_conv1d_optimized(dim, width, extra_state_len, seq_len, has_bias,
                                 silu_activation, itype, has_initial_state):
...
@pytest.mark.parametrize('extra_state_len', [0, 1])
@pytest.mark.parametrize('width', [4])
@pytest.mark.parametrize('dim', [512, 2048])
def test_causal_conv1d_optimized_compare_with_pytorch(
    dim, width, extra_state_len, seq_len, has_bias,
    silu_activation, itype, has_initial_state
):
    """
    Compare optimized kernel with PyTorch reference implementation.
    """
    ...
    if has_initial_state:
        ...
        conv_states_pytorch = conv_states.clone()
    else:
        conv_states = torch.zeros((num_seq, state_len, dim),
                                  device=device,
                                  dtype=itype).transpose(-1, -2)
    ...
    out_pytorch = causal_conv1d_fn_pytorch(
        x,
        weight,
        bias=bias,
        activation=activation,
        conv_states=conv_states_pytorch,
        has_initial_state=has_initial_state_tensor,
        cache_indices=cache_indices,
        query_start_loc=query_start_loc)
